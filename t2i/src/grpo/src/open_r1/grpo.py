...
class GRPOConfig(GRPOConfig):
    ...
    use_importance_sampling: bool = field(default=False, metadata={"help": "if use importance sampling"})
    importance_normalization: str = field(default="softmax", metadata={"help": "the method to normalize importance weights. Options: 'minmax(a,b)', 'rank(x/y)', 'filter(p)'."})
    importance_weighting_type: str = field(default="kl", metadata={"help": "the method to compute importance weights. Options: 'kl', 'log_diff', 'rank_diff'."})
    importance_normalization_temperature: float = field(default=0.1, metadata={"help": "temperature for softmax normalization of importance weights."})
@dataclass
