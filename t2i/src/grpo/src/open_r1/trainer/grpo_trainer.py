...
from utils.importance import compute_importance_weights
...
class JanusT2IR1Trainer(Trainer):
    ...
    def __init__(
        ...
        self.importance_normalization = args.importance_normalization
        self.importance_weighting_type = args.importance_weighting_type
        self.importance_normalization_temperature = args.importance_normalization_temperature
        self.clamp_thresh = 0.0
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.use_importance_sampling:
            with torch.no_grad():
                uncond_inputs_embeds = input_embeds.clone()
                pad_input_embeds = model.module.language_model.get_input_embeddings()(prompt_ids.new_full((1, 1), self.processing_class.pad_id))
                # Match the CFG null branch used during image generation:
                # pad only text conditioning, keep BOS/image-start/image tokens.
                uncond_inputs_embeds[:, 1:prompt_ids.shape[1] - 1] = pad_input_embeds
        ...
        if self.use_importance_sampling:
            # The generation-time unconditional CFG branch attends everywhere.
        ...
        if self.use_importance_sampling:
            with torch.no_grad():
                uncond_per_token_logps = self._get_per_token_logps(
                    model=model.module, 
                    input_embeds=uncond_inputs_embeds,#b,l_txt+l_img(576),c
                    text_ids=None, 
                    img_ids=total_generated_tokens_img, #b,l_img
                    attention_mask=uncond_attention_mask,#b,l_txt+l_img(576)
                    ret_logits=True if self.reward_smooth else False,
                    return_entropy=True
                )
        if self.reward_smooth:
            per_token_logps,per_token_rank_matrix,per_token_entropy=per_token_logps
            uncond_per_token_logps,uncond_per_token_rank_matrix,uncond_per_token_entropy=uncond_per_token_logps
        else:
        ...
        per_token_logps = per_token_logps[:, prompt_length - 1 :]
        if self.use_importance_sampling:
            uncond_per_token_logps = uncond_per_token_logps[:, prompt_length - 1 :]
            response_mask = torch.ones(per_token_logps.shape,device=per_token_logps.device,dtype=torch.bool)
            importance_weight = compute_importance_weights(per_token_logps,uncond_per_token_logps, response_mask=response_mask,
                                                           importance_weighting_type=self.importance_weighting_type, importance_normalization=self.importance_normalization,
                                                           importance_normalization_temperature=self.importance_normalization_temperature)
        ...
        if self.reward_smooth:
            if self.use_importance_sampling:
                ...
                per_token_loss=(per_token_loss * importance_weight).sum() / (importance_weight.sum()+1e-6)
            else:
                B_,C_,H_,W_=quant_b.shape
        ...
        if self.kl_reweight:
            if self.use_importance_sampling:
                fake_cos_sim_map_total = 1 -importance_weight * 2
                kl_mask=0.5+(fake_cos_sim_map_total+1).clamp_(0,1)*0.5
            else:
